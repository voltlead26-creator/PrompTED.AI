...
import {
  AuthError,
  guardRequest as defaultGuardRequest,
} from "../_shared/auth-guard.ts";
import { USER_SAFE_ERROR } from "../_shared/provider-router.ts";
import { buildSystemPrompt, capturePromptProfileSelectionContext, type PromptProfileSelectionContext } from "../_shared/prompt-builder.ts";
import { type DocumentIntelligenceProfile, selectProfile } from "../_shared/document-intelligence-profiles.ts";
import { loadUserMemoryContext as defaultLoadUserMemoryContext } from "../_shared/user-memory.ts";
import {
  admitDocumentTemplate,
  type AdmittedDocumentTemplate,
  applyPreFill,
  DocumentTemplateAdmissionError,
  LEGACY_TEMPLATE_POLICY_VERSION,
  type ProfileData,
  resolveLegacyTemplateForPolicyComparison,
  sectionListInstruction,
  type TemplateSection,
} from "../_shared/template-engine.ts";
import { runDocumentPipeline as defaultRunDocumentPipeline } from "../_shared/document-pipeline.ts";
import { assertDeliverableSections } from "../_shared/document-delivery-guard.ts";
import {
  BESPOKE_DESIGN_VALIDATOR_VERSION,
  designBespokeTemplate as defaultDesignBespokeTemplate,
} from "../_shared/section-designer.ts";
import {
  type DocumentProfilePolicy,
  resolveDocumentProfilePolicy,
} from "../_shared/document-profile-projection.ts";
import {
  ...
  readDocumentAllowanceReplay as defaultReadDocumentAllowanceReplay,
  RECONCILIATION_REQUIRED_PAYLOAD,
  releaseDocumentAllowance as defaultReleaseDocumentAllowance,
  reserveDocumentAllowance as defaultReserveDocumentAllowance,
  resolveAllowanceRequestIdentity,
  settleDocumentAllowance as defaultSettleDocumentAllowance,
...
import { resolveRetainedUploadContext as defaultResolveRetainedUploadContext } from "../_shared/upload-context.ts";
import {
...
export interface GenerateDocumentDependencies {
  guardRequest: typeof defaultGuardRequest;
  loadUserMemoryContext: typeof defaultLoadUserMemoryContext;
  resolveRetainedUploadContext: typeof defaultResolveRetainedUploadContext;
  reserveDocumentAllowance: typeof defaultReserveDocumentAllowance;
  readDocumentAllowanceReplay: typeof defaultReadDocumentAllowanceReplay;
  releaseDocumentAllowance: typeof defaultReleaseDocumentAllowance;
  settleDocumentAllowance: typeof defaultSettleDocumentAllowance;
  designBespokeTemplate: typeof defaultDesignBespokeTemplate;
  runDocumentPipeline: typeof defaultRunDocumentPipeline;
}
...
function replayResponse(
  ...
  const result = reservation.replayResult;
  const events = result?.payload.events;
  if (!result || result.transport !== "sse" || !Array.isArray(events)) {
...
function executionPolicyDigest(
  admission: AdmittedDocumentTemplate,
  template: ReturnType<typeof applyPreFill>,
  profilePolicy: DocumentProfilePolicy | null,
  bespokeProfileSelection?: PromptProfileSelectionContext,
): Promise<string> {
  return allowanceRequestSha256("generate-document:execution-policy-v1", {
    version: LEGACY_TEMPLATE_POLICY_VERSION,
    mode: admission.mode,
    destinations: admission.destinations,
    profilePolicy,
    ...(admission.mode === "bespoke" ? { bespokeProfileSelection } : {}),
    bespokeDesignValidatorVersion: admission.mode === "bespoke"
      ? BESPOKE_DESIGN_VALIDATOR_VERSION
      : null,
    template: {
      ...template,
...
function templateRecoveryResponse(origin: string | null): Response {
  return jsonResponse(
    {
      error: {
...
export async function handleGenerateDocument(
  req: Request,
  dependencies: Partial<GenerateDocumentDependencies> = {},
): Promise<Response> {
  const guardRequest = dependencies.guardRequest ?? defaultGuardRequest;
  const loadUserMemoryContext = dependencies.loadUserMemoryContext ??
    defaultLoadUserMemoryContext;
  const resolveRetainedUploadContext =
    dependencies.resolveRetainedUploadContext ??
      defaultResolveRetainedUploadContext;
  const reserveDocumentAllowance = dependencies.reserveDocumentAllowance ??
    defaultReserveDocumentAllowance;
  const readDocumentAllowanceReplay =
    dependencies.readDocumentAllowanceReplay ??
      defaultReadDocumentAllowanceReplay;
  const releaseDocumentAllowance = dependencies.releaseDocumentAllowance ??
    defaultReleaseDocumentAllowance;
  const settleDocumentAllowance = dependencies.settleDocumentAllowance ??
    defaultSettleDocumentAllowance;
  const designBespokeTemplate = dependencies.designBespokeTemplate ??
    defaultDesignBespokeTemplate;
  const runDocumentPipeline = dependencies.runDocumentPipeline ??
    defaultRunDocumentPipeline;
  ...
  let previous: Awaited<ReturnType<typeof readDocumentAllowanceReplay>>;
  try {
    previous = await readDocumentAllowanceReplay(auth.admin, {
      userId: auth.userId,
    ...
    if (req.signal.aborted) return jsonResponse(USER_SAFE_ERROR, 504, origin);
  ...
  if (req.signal.aborted) return jsonResponse(USER_SAFE_ERROR, 504, origin);
  if (previous.state === "settled" && previous.reservation) {
    return replayResponse(previous.reservation, origin);
  }
  if (previous.reconciliationRequired) {
    return jsonResponse(RECONCILIATION_REQUIRED_PAYLOAD, 409, origin);
  }
  let admission: AdmittedDocumentTemplate;
  try {
    admission = admitDocumentTemplate(auth.body ?? {});
  } catch (error) {
    if (error instanceof DocumentTemplateAdmissionError) {
      if (previous.state === "unsettled") {
        return templateRecoveryResponse(origin);
      }
      return jsonResponse(
        {
          error: {
            code: error.code,
            message:
              "The requested document structure does not match its supported template. Reload the workspace before trying again.",
            retryable: false,
          },
        },
        400,
  ...
  const template = applyPreFill(admission.template, body.profile ?? {});
  let profilePolicy: DocumentProfilePolicy | null;
  let bespokeProfileSelection: PromptProfileSelectionContext | undefined;
  let executionPolicySha256: string;
  try {
    profilePolicy = admission.mode === "catalogue"
      ? resolveDocumentProfilePolicy(template)
      : null;
    bespokeProfileSelection = admission.mode === "bespoke"
      ? capturePromptProfileSelectionContext()
      : undefined;
    executionPolicySha256 = await executionPolicyDigest(
      admission,
      template,
      profilePolicy,
      bespokeProfileSelection,
    );
  } catch {
    // An unresolved server contract must never reserve allowance or dispatch.
    return previous.state === "unsettled"
      ? templateRecoveryResponse(origin)
      : jsonResponse(USER_SAFE_ERROR, 500, origin);
  }
  let legacyExecutionPolicySha256: string | undefined;
  if (
    previous.state === "unsettled" && !previous.reservation?.executionPolicy &&
    previous.hasPriorProviderWork
  ) {
    if (admission.mode === "bespoke") return templateRecoveryResponse(origin);
    // Retain the exact old construction only as evidence for an already
    // accepted request. New requests never use these caller-owned policies.
    const historical = resolveLegacyTemplateForPolicyComparison(templateId);
    const requestedSections = requestSections(body);
    const historicalTemplate = applyPreFill({
      ...historical,
      domain: safeDomain(body.domain, historical.domain),
      structureType: safeStructureType(
        body.structure_type,
        historical.structureType,
      ),
      adviceBoundary: safeAdviceBoundary(
        body.advice_boundary,
        historical.adviceBoundary,
      ),
      sections: requestedSections.length
        ? requestedSections
        : historical.sections,
    }, body.profile ?? {});
    legacyExecutionPolicySha256 = await executionPolicyDigest(
      {
        mode: "catalogue",
        template: historicalTemplate,
        destinations: historicalTemplate.sections.map((section) => ({
          canonicalKey: section.key,
          wireKey: section.key,
        })),
      },
      historicalTemplate,
      resolveDocumentProfilePolicy(historicalTemplate, { historical: true }),
    );
    if (legacyExecutionPolicySha256 !== executionPolicySha256) {
      return templateRecoveryResponse(origin);
    }
  }
  if (
    previous.reservation?.executionPolicy &&
    (previous.reservation.executionPolicy.version !==
        LEGACY_TEMPLATE_POLICY_VERSION ||
      previous.reservation.executionPolicy.sha256 !== executionPolicySha256)
  ) return templateRecoveryResponse(origin);
  const destinationKeys = new Map(
    admission.destinations.map((
      destination,
    ) => [destination.canonicalKey, destination.wireKey]),
  );
  const wireKey = (key: string): string => {
    if (admission.mode === "bespoke") return key;
    const destination = destinationKeys.get(key);
    if (!destination) throw new Error("DOCUMENT_TEMPLATE_OUTPUT_SCOPE_INVALID");
    return destination;
  };
  ...
  try {
    reservation = await reserveDocumentAllowance(auth.admin, {
      ...
      executionPolicy: {
        version: LEGACY_TEMPLATE_POLICY_VERSION,
        sha256: executionPolicySha256,
        ...(legacyExecutionPolicySha256
          ? { legacySha256: legacyExecutionPolicySha256 }
          : {}),
      },
      signal: req.signal,
    });
  ...
  if (reservation.replayResult) {
    return replayResponse(reservation, origin);
  }
  ...
  if (req.signal.aborted) {
    await releaseDocumentAllowance(auth.admin, {
      userId: auth.userId,
      reservation,
      releaseCode: "request_cancelled",
    });
    return jsonResponse(USER_SAFE_ERROR, 504, origin);
  }
  ...
  let designedTemplate: Awaited<ReturnType<typeof designBespokeTemplate>> =
    null;
  let designedProfile: DocumentIntelligenceProfile | null = null;
  try {
    if (body.design_bespoke === true) {
      if (!bespokeProfileSelection) throw new Error("BESPOKE_PROFILE_POLICY_REQUIRED");
      designedTemplate = await designBespokeTemplate({
        ...
        systemPrompt: buildSystemPrompt({
          ...
          profileSelection: bespokeProfileSelection,
          extra: "",
        }),
        signal: req.signal,
      });
      if (!designedTemplate) throw new Error("DOCUMENT_DESIGN_REQUIRED");
      // Preserve the pipeline's existing name/id/domain selection, but select
      // only from the candidate contents accepted before the design dispatch.
      designedProfile = selectProfile(
        [designedTemplate.name, designedTemplate.id].filter(Boolean).join("\n"),
        designedTemplate.domain,
        bespokeProfileSelection.profiles,
      );
    }
  } catch (error) {
  ...
  const systemPrompt = buildSystemPrompt({
    ...
    profileHint,
    resolvedProfile: profilePolicy?.profile,
    profileSelection: bespokeProfileSelection,
    extra: [
      ...
      try {
        ...
        try {
          pipelineResult = await runDocumentPipeline({
            ...
            resolvedProfile: designedTemplate ? designedProfile : profilePolicy?.profile,
            situation,
            conversationContext,
            uploadContext,
            extractedText,
        ...
        if (
          reviewedSections.length !== effectiveTemplate.sections.length ||
          reviewedSections.some((section, index) =>
            section.key !== effectiveTemplate.sections[index]?.key
          )
        ) {
          throw new Error("DOCUMENT_TEMPLATE_OUTPUT_SCOPE_INVALID");
        }
        ...
        if (unresolvedPlaceholders.length > 0) {
          responseEvents.push({
            ...
            placeholders: unresolvedPlaceholders.map((placeholder) => ({
              ...placeholder,
              sectionKey: wireKey(placeholder.sectionKey),
            })),
          });
        }
        ...
        if (missingInfo.length > 0) {
          responseEvents.push({
            type: "missing_info",
            sections: missingInfo.map((section) => ({
              ...section,
              key: wireKey(section.key),
            })),
          });
