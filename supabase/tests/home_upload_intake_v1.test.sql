...
select ok(
  pg_temp.require_local_test_sessions(array['home_intake_writer', 'home_intake_reader']),
  'both concurrency sessions are independent backends in this exact database and cluster'
);
select throws_ok(
  $$select pg_temp.require_local_test_sessions(array['home_intake_writer', 'home_intake_writer'])$$,
