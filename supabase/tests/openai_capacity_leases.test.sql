...
select ok(
  pg_temp.require_local_test_sessions(array['capacity_session_a', 'capacity_session_b']),
  'both concurrency sessions are independent backends in this exact database and cluster'
);
select throws_ok(
  $$select pg_temp.require_local_test_sessions(array['capacity_session_a', 'capacity_session_a'])$$,
