...
select ok(
  pg_temp.require_local_test_sessions(array['workspace_snapshot_writer', 'workspace_snapshot_reader']),
  'both concurrency sessions are independent backends in this exact database and cluster'
);
select throws_ok(
  $$select pg_temp.require_local_test_sessions(array['workspace_snapshot_writer', 'workspace_snapshot_writer'])$$,
