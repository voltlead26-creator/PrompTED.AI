-- Business and Premium use one per-user ceiling; old admissions remain immutable.
begin;
...
select pg_temp.plan_user(n),'plan-'||n||'@example.invalid',false,false,now(),
  ...
  '{}',now(),now() from generate_series(1,4) n;
...
create temp table original_plan_subscriptions as select to_jsonb(s) value
  from public.subscriptions s where user_id in (select pg_temp.plan_user(n) from generate_series(1,4) n);

set local role service_role;
select is(public.get_effective_product_access_v1(pg_temp.plan_user(2))->'monthly_document_cap',
  public.get_effective_product_access_v1(pg_temp.plan_user(1))->'monthly_document_cap',
  'Business allowance equals Premium per user');
select is(public.get_effective_product_access_v1(pg_temp.plan_user(1))->'monthly_document_cap',
  '40'::jsonb,'existing enforced Premium allowance remains unchanged pending the numeric-cap decision');
select is(public.get_effective_product_access_v1(pg_temp.plan_user(2))->>'subscription_plan',
  'business','equal allowances do not rewrite Business subscription identity');
...
select is(public.reserve_document_allowance(pg_temp.plan_user(2),'business-first','generate-document',
  repeat('a',64),'business',1000,1800)->'monthly_cap','40'::jsonb,
...
select is(public.reserve_document_allowance(pg_temp.plan_user(1),'premium-first','generate-document',
  ...
  'Premium admission uses the same independently resolved limit');
reset role;
select is((select monthly_cap from private.document_plan_snapshot(pg_temp.plan_user(2))),40,
...
select is(public.reserve_document_allowance(pg_temp.plan_user(4),'historical-business','generate-document',
  repeat('a',64),'business',40,1800)->'provider_permitted','false'::jsonb,
...
select is(public.reserve_document_allowance(pg_temp.plan_user(4),'new-business','generate-document',
  repeat('b',64),'business',1000,1800)->'monthly_cap','40'::jsonb,
...
from private.document_allowance_reservations cross join generate_series(1,39) n
...
select throws_ok($$select public.reserve_document_allowance(pg_temp.plan_user(2),'business-over-cap',
  ...
  'P0001','ALLOWANCE_CAP_REACHED','Business cannot admit a 41st concurrent document with a stale higher cap');
select is(public.reserve_document_allowance(pg_temp.plan_user(2),'business-first','generate-document',
...
select is((select count(*)::integer from public.usage_ledger where user_id in
  (select pg_temp.plan_user(n) from generate_series(1,4) n)),0,
...
select is((select jsonb_agg(value order by value->>'user_id') from original_plan_subscriptions),
  (select jsonb_agg(to_jsonb(s) order by user_id) from public.subscriptions s where user_id in
    (select pg_temp.plan_user(n) from generate_series(1,4) n)),
...
set local role service_role;
select is(public.get_effective_product_access_v1(pg_temp.plan_user(2))->'monthly_document_cap',
  '40'::jsonb,'trialling Business receives the same active allowance');
reset role;
