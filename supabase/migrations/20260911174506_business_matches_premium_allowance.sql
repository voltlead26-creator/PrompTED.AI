-- Owner decision: Business matches Premium per user. Preserve subscriptions,
-- historical usage and immutable reservations; only new access resolution changes.
begin;
...
begin
  ...
  v_owner := coalesce(
    ...
    false);
  -- Business has Premium's per-user allowance plus business capabilities.
  -- Retain the currently enforced Free/Pro/Premium ceilings; the owner's
  -- separate 1,000-document overlay is never a commercial Business allowance.
  v_cap := case when v_owner then 1000
    when v_effective_plan = 'pro' then 20
    when v_effective_plan in ('premium','business') then 40
    else 3 end;
