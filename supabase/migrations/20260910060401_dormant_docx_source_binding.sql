-- Dormant, service-only binding of an immutable DOCX checkpoint to the existing
-- aggregate. Literal source text is never admitted to the legacy HTML editor.
begin;

create table private.docx_source_bindings (
  ...
  units_sha256 text not null check (units_sha256 ~ '^[0-9a-f]{64}$'),
  -- Identity projection only. Mutable wording has one home: public.sections.
  section_bindings jsonb not null check (jsonb_typeof(section_bindings) = 'array'),
  created_at timestamptz not null default now()
);
alter table private.docx_source_bindings enable row level security;
revoke all on table private.docx_source_bindings from public, anon, authenticated, service_role;

create function public.prepare_docx_source_document_v1(
...
begin
  ...
  for v_paragraph, v_index in select value, ordinality::integer
    ...
    if jsonb_typeof(v_paragraph) is distinct from 'object'
      ...
      or v_paragraph->>'id' is distinct from 'p:' || v_index::text
...
$function$;
revoke all on function public.prepare_docx_source_document_v1(uuid,uuid,text,jsonb)
  from public, anon, authenticated;
grant execute on function public.prepare_docx_source_document_v1(uuid,uuid,text,jsonb) to service_role;

create function private.is_dormant_docx_resource(p_kind text,p_id uuid)
returns boolean language sql stable security definer set search_path = '' as $function$
  select exists (select 1 from private.docx_source_bindings b
    where b.user_id = auth.uid() and (
      (p_kind = 'document' and b.document_id = p_id)
      or (p_kind = 'outcome' and b.outcome_id = p_id)))
$function$;
revoke all on function private.is_dormant_docx_resource(text,uuid) from public, anon, service_role;
grant execute on function private.is_dormant_docx_resource(text,uuid) to authenticated;

create policy docx_source_documents_dormant on public.documents as restrictive
  for all to authenticated using (not private.is_dormant_docx_resource('document',id))
  with check (not private.is_dormant_docx_resource('document',id));
create policy docx_source_sections_dormant on public.sections as restrictive
  for all to authenticated using (not private.is_dormant_docx_resource('document',document_id))
  with check (not private.is_dormant_docx_resource('document',document_id));
create policy docx_source_outcomes_dormant on public.outcomes as restrictive
  for all to authenticated using (not private.is_dormant_docx_resource('outcome',id))
  with check (not private.is_dormant_docx_resource('outcome',id));

create function private.guard_dormant_docx_source() returns trigger
language plpgsql security definer set search_path = '' as $function$
declare v_bound boolean; v_old_document_id uuid; v_new_document_id uuid;
begin
  if tg_table_name = 'docx_source_bindings' then
    if tg_op = 'DELETE' and (not exists(select 1 from auth.users where id=old.user_id)
      or not exists(select 1 from public.documents where id=old.document_id)
      or not exists(select 1 from public.outcomes where id=old.outcome_id)) then return old; end if;
    raise exception 'DOCX_SOURCE_BINDING_IMMUTABLE';
  elsif tg_table_name = 'sections' then
    if tg_op = 'DELETE' and not exists(select 1 from public.documents where id=old.document_id)
      then return old; end if;
    if tg_op <> 'INSERT' then v_old_document_id := old.document_id; end if;
    if tg_op <> 'DELETE' then v_new_document_id := new.document_id; end if;
    select exists(select 1 from private.docx_source_bindings b
      where b.document_id in (v_old_document_id,v_new_document_id)) into v_bound;
  elsif tg_table_name = 'documents' then
    if tg_op = 'DELETE' and not exists(select 1 from auth.users where id=old.user_id)
      then return old; end if;
    select exists(select 1 from private.docx_source_bindings where document_id=old.id) into v_bound;
  else
    if tg_op = 'DELETE' and not exists(select 1 from auth.users where id=old.user_id)
      then return old; end if;
    select exists(select 1 from private.docx_source_bindings where outcome_id=old.id) into v_bound;
  end if;
  if v_bound then raise exception 'DOCX_SOURCE_EDITING_NOT_ACTIVATED'; end if;
  if tg_op = 'DELETE' then return old; end if;
  return new;
end;
$function$;
revoke all on function private.guard_dormant_docx_source() from public, anon, authenticated, service_role;
create trigger docx_source_binding_immutable before update or delete on private.docx_source_bindings
  for each row execute function private.guard_dormant_docx_source();
create trigger docx_source_section_guard before insert or update or delete on public.sections
  for each row execute function private.guard_dormant_docx_source();
-- Until activation supplies a deletion/tombstone contract, a normal deletion
-- cannot erase replay identity and cause a delayed prepare retry to resurrect it.
-- Account erasure remains supported through its existing parent cascade.
create trigger docx_source_document_guard before update or delete on public.documents
  for each row execute function private.guard_dormant_docx_source();
create trigger docx_source_outcome_guard before update or delete on public.outcomes
  for each row execute function private.guard_dormant_docx_source();

-- Existing security-definer hydration/export commands must obey the same
-- source-only boundary; changing RLS alone would not protect these readers.
alter function public.get_workspace_snapshot_v1(uuid,uuid) set schema private;
revoke all on function private.get_workspace_snapshot_v1(uuid,uuid) from public,anon,authenticated,service_role;
create function public.get_workspace_snapshot_v1(p_outcome_id uuid,p_active_section_id uuid default null)
returns jsonb language plpgsql stable security definer set search_path = '' as $function$
begin
  if private.is_dormant_docx_resource('outcome',p_outcome_id) then
    raise exception 'DOCX_SOURCE_EDITING_NOT_ACTIVATED'; end if;
  return private.get_workspace_snapshot_v1(p_outcome_id,p_active_section_id);
end;
$function$;
revoke all on function public.get_workspace_snapshot_v1(uuid,uuid) from public,anon,service_role;
grant execute on function public.get_workspace_snapshot_v1(uuid,uuid) to authenticated;

alter function public.get_workspace_section_body_v1(uuid,uuid,integer,integer) set schema private;
revoke all on function private.get_workspace_section_body_v1(uuid,uuid,integer,integer) from public,anon,authenticated,service_role;
create function public.get_workspace_section_body_v1(p_outcome_id uuid,p_section_id uuid,
  p_expected_document_revision integer,p_expected_section_revision integer)
returns jsonb language plpgsql stable security definer set search_path = '' as $function$
begin
  if private.is_dormant_docx_resource('outcome',p_outcome_id) then
    raise exception 'DOCX_SOURCE_EDITING_NOT_ACTIVATED'; end if;
  return private.get_workspace_section_body_v1(p_outcome_id,p_section_id,
    p_expected_document_revision,p_expected_section_revision);
end;
$function$;
revoke all on function public.get_workspace_section_body_v1(uuid,uuid,integer,integer) from public,anon,service_role;
grant execute on function public.get_workspace_section_body_v1(uuid,uuid,integer,integer) to authenticated;

alter function public.load_legacy_export_snapshot(uuid,uuid,uuid) set schema private;
revoke all on function private.load_legacy_export_snapshot(uuid,uuid,uuid) from public,anon,authenticated,service_role;
create function public.load_legacy_export_snapshot(p_user_id uuid,p_document_id uuid,p_artifact_id uuid)
returns jsonb language plpgsql stable security definer set search_path = '' as $function$
begin
  if exists(select 1 from private.docx_source_bindings
    where document_id=p_document_id and user_id=p_user_id) then
    raise exception 'DOCX_SOURCE_EXPORT_NOT_ACTIVATED'; end if;
  return private.load_legacy_export_snapshot(p_user_id,p_document_id,p_artifact_id);
end;
$function$;
revoke all on function public.load_legacy_export_snapshot(uuid,uuid,uuid) from public,anon,authenticated;
grant execute on function public.load_legacy_export_snapshot(uuid,uuid,uuid) to service_role;

comment on table private.docx_source_bindings is
  'Dormant immutable DOCX source identities; current literal wording remains in public.sections. No active editor/export authority.';
commit;
