-- Extract the existing legacy save implementation for later trusted atomic
-- generation finalization. Public API, owner, grants, hashes, receipts, locks,
-- revisions and trigger behavior stay unchanged. This migration does not
-- activate generation attachment, allowance settlement or source-only editing.

begin;

-- Fail closed on an unexpected installed implementation before replacing it.
-- This transaction-local catalog snapshot is dropped at commit or rollback.
create temporary table pg_temp.legacy_workspace_core_v1_public_identity
on commit drop as
select p.oid as function_oid, p.proowner as function_owner,
  pg_catalog.to_jsonb(p) - 'prosrc' as identity,
  pg_catalog.encode(extensions.digest(pg_catalog.convert_to(p.prosrc,'UTF8'),'sha256'),'hex') as body_sha256
from pg_catalog.pg_proc p
where p.oid='public.save_own_legacy_workspace_v1(text,uuid,uuid,integer,jsonb,jsonb,jsonb)'::regprocedure;

do $migration$
declare v_before record;
begin
  select * into strict v_before from pg_temp.legacy_workspace_core_v1_public_identity;
  if v_before.body_sha256 is distinct from
    '6a7eb1228611c35c64720b3d3c2654f039d510c797010739b34046639fa5897b' then
    raise exception 'LEGACY_WORKSPACE_CORE_PREDECESSOR_CHANGED';
  end if;
end;
$migration$;

-- The reviewed predecessor body below differs only in its actor initializer.
create function private.save_legacy_workspace_core_v1(
...
revoke all on function private.save_legacy_workspace_core_v1(
  uuid, text, uuid, uuid, integer, jsonb, jsonb, jsonb
) from public, anon, authenticated, service_role;

-- The invoker core executes as its trusted caller. Give it the same owner as
-- the existing public definer, even if the migration executor is different.
do $migration$
declare
  v_before record;
  v_owner_name name;
  v_core oid := 'private.save_legacy_workspace_core_v1(uuid,text,uuid,uuid,integer,jsonb,jsonb,jsonb)'::regprocedure;
  v_core_owner oid;
  v_core_body_sha256 text;
begin
  select * into strict v_before from pg_temp.legacy_workspace_core_v1_public_identity;
  select r.rolname into strict v_owner_name from pg_catalog.pg_roles r
    where r.oid=v_before.function_owner;
  execute pg_catalog.format(
    'alter function private.save_legacy_workspace_core_v1(uuid,text,uuid,uuid,integer,jsonb,jsonb,jsonb) owner to %I',
    v_owner_name
  );
  select p.proowner,
    pg_catalog.encode(extensions.digest(pg_catalog.convert_to(p.prosrc,'UTF8'),'sha256'),'hex')
    into strict v_core_owner, v_core_body_sha256
  from pg_catalog.pg_proc p where p.oid=v_core;
  if v_core_owner is distinct from v_before.function_owner
    or not pg_catalog.has_function_privilege(v_before.function_owner,v_core,'EXECUTE') then
    raise exception 'LEGACY_WORKSPACE_CORE_OWNER_INVALID';
  end if;
  if exists (
    select 1 from pg_catalog.pg_proc p,
      pg_catalog.aclexplode(coalesce(p.proacl,pg_catalog.acldefault('f',p.proowner))) permission
    where p.oid=v_core and permission.privilege_type='EXECUTE'
      and permission.grantee<>p.proowner
  ) then
    -- Do not change unknown default privileges or publish an exposed actor API.
    raise exception 'LEGACY_WORKSPACE_CORE_PRIVILEGES_INVALID';
  end if;
  if v_core_body_sha256 is distinct from
    'cbebe21e2b42c68b96ee588075889c1c2f83e90ca7d644e59d5de1164057a756' then
    raise exception 'LEGACY_WORKSPACE_CORE_BODY_CHANGED';
  end if;
end;
$migration$;

-- Keep the existing function OID and ACL. The actor still comes from auth.uid().
...
as $function$
begin
  return private.save_legacy_workspace_core_v1(
    auth.uid(), p_idempotency_key, p_outcome_id, p_document_id,
    p_expected_document_revision, p_expected_document, p_document, p_sections
  );
end;
$function$;

-- Only the function body may change. This includes the original OID, owner,
-- ACL, language, argument names/defaults, null behavior, search path and flags.
do $migration$
declare v_before record; v_after jsonb;
begin
  select * into strict v_before from pg_temp.legacy_workspace_core_v1_public_identity;
  select pg_catalog.to_jsonb(p) - 'prosrc' into v_after
    from pg_catalog.pg_proc p where p.oid=v_before.function_oid;
  if v_after is distinct from v_before.identity then
    raise exception 'LEGACY_WORKSPACE_PUBLIC_CONTRACT_CHANGED';
  end if;
end;
$migration$;
